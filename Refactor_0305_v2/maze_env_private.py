...
class Maze_pravite(tk.Tk, object):
    ...
    def _build_maze(self):  # 创建初始画布
        ...
        obstacle_center = np.empty((10, 10, 2))
        for i in range(len(ed.obstacle)):
            for j in range(len(ed.obstacle[i])):
                b = np.array([UNIT * ed.obstacle[i][j][0], UNIT * ed.obstacle[i][j][1]])
                obstacle_center[i][j] = b
                self.hell[i][j] = self.canvas.create_rectangle(
                    obstacle_center[i][j][0] - block_size, obstacle_center[i][j][1] - block_size,
                    obstacle_center[i][j][0] + block_size, obstacle_center[i][j][1] + block_size,
        ...
        # create oval
        self.oval_center = self.origin + np.array([UNIT * 19, UNIT * 19])
        ...
        # create red rect
        self.agent_center = self.origin + np.array([UNIT * 1, UNIT * 1])
