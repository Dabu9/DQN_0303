...
class DeepQNetwork:
    ...
    def choose_action(self, observation):
        ...
        if np.random.uniform() < self.epsilon:
            ...
            if observation.equal(a):
                self.actions_value_cl.append(
                    [actions_value[0][1].detach().numpy(), actions_value[0][2].detach().numpy(),
                     actions_value[0][5].detach().numpy()])
            # 选择最大动作价值对应的action
    ...
    def plot_results(self, steps, cost, flag_step, fail_times, action_value, distance):
        ...
        plt.xlabel('第x次成功', fontproperties=font)
        plt.ylabel('距离', fontproperties=font)
        plt.title('距离变化曲线', fontproperties=font)
