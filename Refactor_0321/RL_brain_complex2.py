...
class Net(nn.Module):
    def __init__(self, n_feature, n_hidden1, n_hidden2,n_hidden3, n_output):
        ...
        self.el3 = nn.Linear(n_hidden2, n_hidden3)  # evaluate net
        self.q = nn.Linear(n_hidden3, n_output)  # q_target net
    ...
    def forward(self, x):
        ...
        x = F.relu(x)
        x = self.el3(x)  # pass through additional hidden layer
        x = F.relu(x)
        x = self.q(x)
...
class DeepQNetwork:
    ...
    def _build_net(self):
        self.q_eval = Net(self.n_features, self.n_hidden1, self.n_hidden2,  self.n_hidden3, self.n_actions)
        self.q_target = Net(self.n_features, self.n_hidden1, self.n_hidden2,  self.n_hidden3, self.n_actions)
