...
class Net(nn.Module):
    def __init__(self, n_feature, n_hidden1, n_hidden2, n_hidden3, n_hidden4, n_hidden5, n_output):
        ...
        self.fc6 = nn.Linear(n_hidden5, n_output)
...
class DeepQNetwork:
    def __init__(self, n_actions, n_features, n_hidden1=6, n_hidden2=12, n_hidden3=12, n_hidden4=64, n_hidden5=64, learning_rate=0.01, reward_decay=0.9, e_greedy=0.9,
        ...
        # Add important parameters that you want to save
        self.params_to_save = {'n_actions': self.n_actions,
                               ...
                               'e_greedy': self.epsilon_max,
                               'replace_target_iter': self.replace_target_iter}

    def _build_net(self):
        self.q_eval = Net(self.n_features, self.n_hidden1, self.n_hidden2, self.n_hidden3, self.n_hidden4, self.n_hidden5, self.n_actions)
        self.q_target = Net(self.n_features, self.n_hidden1, self.n_hidden2, self.n_hidden3, self.n_hidden4, self.n_hidden5, self.n_actions)
    ...
    def save_params(self, sub_dir):
        # Save important parameters to a file in the sub-directory
        file_path = f"{sub_dir}/params.txt"
        with open(file_path, 'w') as f:
            for key, val in self.params_to_save.items():
                f.write(f"{key}: {val}\n")
    ...
    def plot_results(self, steps, cost, fail_times, action_value):
        ...
        sub_dir = f"{output_dir}/{time_now}"
        os.makedirs(sub_dir)

        # Save the first figure as SVG format
        f1 = plt.gcf()
        plt.savefig(f"{sub_dir}/episode.svg", format="svg")
        plt.close(f1)

        # Save the second figure as SVG format
        f2 = plt.figure(figsize=(8, 6))
        plt.plot(np.arange(len(fail_times)), fail_times)
        plt.savefig(f"{sub_dir}/failtimes.svg", format="svg")

        # Save the third figure as SVG format
        f3 = plt.figure(figsize=(8, 6))
        plt.plot(np.arange(len(action_value)), action_value)
        plt.legend(['down', 'right', 'right-down'])
        plt.savefig(f"{sub_dir}/actionvalue.svg", format="svg")

        # Save the fourth figure as SVG format
        ...
        plt.ylabel("Count")
        plt.title("Max Column Counts")
        plt.savefig(f"{sub_dir}/maxcolcounts.svg", format="svg")

        # Save the fifth figure as SVG format
        f5 = plt.figure(figsize=(8, 6))
        plt.plot(np.arange(len(self.epsilon_arr)), self.epsilon_arr)
        plt.savefig(f"{sub_dir}/epsilonarr.svg", format="svg")

        self.save_params(sub_dir)
        plt.show()
